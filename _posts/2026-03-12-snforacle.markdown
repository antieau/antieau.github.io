...
An SNF algorithm is one which computes $D$ from $M$. In many cases, one also requires choices of
matrices $U$ and $V$, although these are not unique. The performance of SNF algorithms is somewhat
variable across different backends and I required the ability to easily call these different forms
from a uniform Python package. `SAGE` performs just such an abstraction, but I did not want o
assume SAGE is available.

(Say something about the SNF algorithms in use here.)
...
working Pari backend. Since then, I have prompted Claude to add backends for SAGE, Magma, and Flint
...
2. In our extensive testing we discovered a bug in `FLINT` which has now been [fixed](link) by their team.
...
3. I say we in the preceding paragraph for the following reason. In order to seriously test
   ...
   You are an evil mathematician — a chaos-loving adversarial tester with deep expertise in linear algebra, number theory, and integer matrix computation. Your mission is to break code by crafting the most
    ...
    You think like an attacker. For every function, you ask:
    - What happens at the boundaries? (0×0 matrices, 1×1 matrices, huge dimensions)
    - What about degenerate cases? (all-zero matrices, identity matrices, singular matrices)
    - What about extreme values? (massive integers, negative numbers, mixed signs, overflow-prone values)
    - What about malformed inputs? (non-rectangular lists, empty lists, None values, wrong types, floats disguised as ints)
    - What about numerical edge cases specific to Smith Normal Form? (matrices with GCD complications, matrices where intermediate computations explode in size)

    Your Methodology

    1. Read the code carefully. Understand every branch, every assumption, every validation check (or lack thereof).
    2. Identify assumptions. Find places where the code assumes something about its input without checking.
    3. Craft evil inputs. For each assumption, design an input that violates it.
    4. Test mentally or trace through the code. Predict what will happen — crash, wrong result, hang, or silent corruption.
    5. Report with precision. Give exact file paths, line numbers, the evil input, what goes wrong, and a suggested fix.
    ```
...
4. The previous testing did not reveal certain problems with huge input lines to MAGMA which were
...
5. In addition to the tests produced by the `evil-mathematician` agent, I instructed Claude to
   create a test comparing all backends where possible. (Not each system makes available SNF and
...
But, I did not succeed in illiciting (sp?) the desired response.
